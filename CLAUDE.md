...
- `components/data-table/*` (TanStack Table v8 + Virtual; row ids are URLs), `components/charts/` (16 charts, 4 lazy tabs, **no barrel `index.ts`**), `components/game-thumb.tsx` (`lib/thumbnail.ts` → `/img/160|640/...` on the web, the original URL in Tauri), `components/legal-gate.tsx` (links flagged `inGate` in `lib/about.ts`; re-prompts when `LEGAL_VERSION` in `stores/prefs.ts` changes).
...
- Code split (`vite.config.ts` rolldown `codeSplitting.groups`): `vendor-react` (priority 100 — must claim React first), `vendor-query`, `vendor-ui`. There is deliberately **no vendor-charts group** (it pulled React/clsx in and put Recharts on the first load); Recharts stays in the lazy chart chunks. Initial load ≈ 195 KB gzip. Web build `target` is Safari/iOS 15.4 (Radix class static blocks otherwise break iOS 15); Tauri uses the default.
...
20. **D1 bindings have no `database_id`**: `wrangler deploy` binds (or creates) the database by name. Migrations are applied by hand (`--remote`), never in the build.
...
`main` = v3.9 line plus the pending v4 PRs: **#81** (Phase 0 hotfix: real build on Cloudflare, security headers, Android CI) and **#82** (Phase 1: rotating refresh / patch pipeline, CI hardening; commits 304b4d3, bbad450, 7b3882b). Branch **`v4`** (stacked on #82, not pushed yet) holds Phase 2 (Worker platform, read-only app), the web review fixes, Phase 4 (D1 queue, admin, ingest, suggest, RSS discovery) and Phase 5 (licenses, policies, docs, version 4.0.0). Already in v4 from Phase 3: 18+ games hidden by default with a Settings opt-in, the age line in the legal gate, `/removed`, `<html lang>`. **Still to do (Phase 3 visual redesign, awaiting mockup approval):** Tailwind v4, top nav instead of the sidebar, a Welcome page at `/`, the legal gate as an overlay, URL-synced filters. Go-live = one merge of `v4` into `main` after staging checks, then tag `v4.0.0`.
