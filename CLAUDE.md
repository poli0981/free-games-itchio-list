...
`main` = v3.9 line plus the merged v4 PRs **#81** (Phase 0 hotfix: real build on Cloudflare, security headers, Android CI) and **#82** (Phase 1: rotating refresh / patch pipeline, CI hardening), both squash-merged. Branch **`v4`** (rebased onto that `main`, not pushed yet) holds Phase 2 (Worker platform, read-only app), the web review fixes, Phase 4 (D1 queue, admin, ingest, suggest, RSS discovery) and Phase 5 (licenses, policies, docs, version 4.0.0). Phase 3 (the approved redesign) is in `v4` too: Tailwind v4, top nav + footer instead of the sidebar, the Welcome page at `/`, the legal gate as an overlay, URL-synced Games filters, the new detail / removed / settings pages and the "Free Itch Games" brand (icons, OG image); earlier Phase 3 pieces: 18+ hidden by default, the age line in the gate, `/removed`, `<html lang>`. Go-live = one merge of `v4` into `main` after staging checks, then tag `v4.0.0`.
